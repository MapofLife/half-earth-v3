...
function Blah() {
  ...
  useEffect(() => {
    t(
      'On the species page, you can search for any bird, mammal, reptile, or amphibian species as well as growing lists for fish, butterflies, moths, ants, dragonflies, cacti, conifers, palms, and other plants and visit their page. Here, you can view the Data Layers tab and the Indicator Scores tab.'
    );
    t(
      "Here, all available spatial datasets are listed for the selected species. They are divided between publicly available data sourced from Map of Life and private data sourced from your organization's ArcGIS database. Additionally, there are a set of regional layers, including administrative boundaries and protected areas. These layers can be enabled to display on the map and further explored."
    );
    t(
      'Here, you can view and explore the selected species Protection and Habitat Scores. A graph of change over time in the score value is displayed along with a table. The table displays information about the species protection or habitat scores in all countries that species lives in. In the drop down menu, you can select which country you want to compare the conservation status of the species with. In the screenshot below, we see that the species has a protection score of 95 in the NBIS country, whereas in a comparison country it has a score of 75, and globally, the species has a score of 100. Changing the comparison country will also update the graph, so you can view how the protection score of the species has changed over time in a different country compared to the NBIS country.'
    );
    t(
      "On the Regions page, you can select specific areas within the NBIS country to automatically generate a list of species that are likely to be found in the area. Currently, you can choose to display on the map Protected Areas, Provinces, or other country-specific regions, then you can select the specific area you're interested in on the map. You also have the ability to directly draw a custom area on the map or upload the shapefile, and any other region types that are of interest to your organization can be uploaded to your ArcGIS database and added to the list here."
    );
    t(
    ...
    t(
      'For all bird, mammal, amphibian, and reptile species, we set a target of how much of their range area should ideally be protected, and each species gets an individual protection score from 0 to 100 based on how close they are to meeting this target. A frog that lives in a specific mountain, for example, has a small range, so we set a high target for what percent of its habitat should be protected – 80 or 90 or even 100% if the total range area is very small. On the other hand, species like a wide-ranging mammal have larger range areas, so they have a lower target of what percent of their range should be protected - 20 or 30%, perhaps.'
    );
    t(
      'All these species protection scores, across all species that live in the country, are averaged to get the overall Species Protection Index for the NBIS country. If, for example, that number was 62 that tells us that on average, the species of the country are 62% of the way toward meeting their protection goals.'
    );
    t(
    ...
    t(
      'In both the national and province level views, we can explore the breakdown of the individual protection scores across species. This analysis is visualized as a histogram, where each bar represents a range of protection score values and shows how many species have protection scores in that range, along with the breakdown by taxonomic group. You can click a bin and get the list of species with that score in the region in the score distribution column to the left.'
    );
    t(
    ...
    t(
      'The habitat scores across all the species that live in the country are averaged to get the overall Species Habitat Index for the NBIS country. If, for example, that number was 98.6 this tells us that on average, the species of that country have lost 1.4% of their suitable habitat since 2001.'
    );
    t(
    ...
    t(
      'The information scores across all the species that live in the country are averaged to get the overall Species Information Index. A value of 10, for example, tells us that on average, the species of the country had a documented occurrence point in 10% of their range area in 2023.'
    );
    t(
